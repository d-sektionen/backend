...
def send_new_entry_mails(creator: User, entry: BudgetEntry):
    ...
    treasurer_body = f"Hej!\n{creator.get_full_name()} har fyllt ut ett nytt personligt utlägg som gäller ditt utskott. Gå in och granska det här: [länk]"  # TODO: fill in link
    ...
    if deg_committee:
        ...
        deg_body = f"Hej!\nDet finns ett nytt personligt utlägg för {entry.committee.name} för dig att granska och bokföra, du hittar utlägget här: [länk]"  # TODO: fill in link
        deg_emails = [f"{x.first_name}.{x.last_name}@d-sektionen.se" for x in deg_committee.members.all()]
...
def send_unpayed_entries_mail(entry: BudgetEntry):
    ...
    subject = f"{unpayed_entry_count} bokförda utlägg finns att betala ut"
    body = f"Hej! Det finns {unpayed_entry_count} nya bokförda personliga utlägg för dig att betala ut. Du kommer åt dem här: [länk]"  # TODO: fill in link
