...
class CommitteeForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if "treasurer_email" not in self.changed_data and "treasurer" in self.changed_data:
        ...
        if self.cleaned_data.get("treasurer") not in self.cleaned_data.get("members"):
            raise forms.ValidationError({"treasurer": "Contact needs to be a member of the committee"})  # TODO: should not need to be a member of the committee
        return super().clean()